...
class CarDatabaseApp:
    ...
    def load_data(self):
        self.conn = sqlite3.connect("epood_kvare.db")
        self.cur = self.conn.cursor()
        self.cur.execute("CREATE TABLE IF NOT EXISTS kvare (id INTEGER PRIMARY KEY AUTOINCREMENT, first_name VARCHAR(255) NOT NULL, last_name VARCHAR(255) NOT NULL, email VARCHAR(255) NOT NULL, car_make VARCHAR(255) NOT NULL, car_model VARCHAR(255) NOT NULL, car_year INTEGER, car_price DECIMAL(10,2))")
        self.conn.commit()
        self.show_data()
    
    def show_data(self):
        offset = (self.page - 1) * 20
        self.cur.execute("SELECT * FROM kvare LIMIT 20 OFFSET ?", (offset,))
    ...
    def search(self, event=None):
        ...
        self.cur.execute("SELECT * FROM kvare WHERE first_name LIKE ? OR last_name LIKE ? OR email LIKE ? OR car_make LIKE ? OR car_model LIKE ?", 
    ...
    def add_data(self):
        ...
        self.cur.execute("INSERT INTO kvare (first_name, last_name, email, car_make, car_model, car_year, car_price) VALUES (?, ?, ?, ?, ?, ?, ?)",
        ...
        if len(selected_item) == 0:
            ...
            if result:
                for item in selected_item:
                    self.cur.execute("DELETE FROM kvare WHERE id=?", (self.tree.item(item, "text"),))
                    self.conn.commit()
                self.show_data()
    ...
    def update_data(self):
        ...
        if len(selected_item) == 0:
            ...
            for item in selected_item:
                self.cur.execute("UPDATE kvare SET first_name=?, last_name=?, email=?, car_make=?, car_model=?, car_year=?, car_price=? WHERE id=?", 
    ...
    def show_stats(self):
        self.cur.execute("SELECT COUNT(*) FROM kvare")
        total_rows = self.cur.fetchone()[0]
        
        self.cur.execute("SELECT COUNT(DISTINCT car_make) FROM kvare")
        total_car_makes = self.cur.fetchone()[0]
        
        self.cur.execute("SELECT AVG(car_price) FROM kvare")
